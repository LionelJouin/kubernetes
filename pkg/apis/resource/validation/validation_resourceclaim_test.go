...
func TestValidateClaimStatusUpdate(t *testing.T) {
	...
	scenarios := map[string]struct {
				...
				claim.Status.Devices = []resource.AllocatedDeviceStatus{
					{
						...
						NetworkData: resource.NetworkDeviceData{
							InterfaceName: "net-1",
							Addresses: []resource.NetworkAddress{
								{CIDR: "10.9.8.0/24"},
								{CIDR: "2001:db8::/64"},
							},
		...
		"invalid-network-device-status": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("status", "devices").Index(0).Child("networkData", "addresses").Index(0).Child("cidr"), "300.9.8.0/24", "must be a valid CIDR value, (e.g. 10.9.8.0/24 or 2001:db8::/64)"),
			},
			oldClaim: func() *resource.ResourceClaim { return validAllocatedClaim }(),
			update: func(claim *resource.ResourceClaim) *resource.ResourceClaim {
				claim = claim.DeepCopy()
		...
		"invalid-data-device-status": {
			wantFailures: field.ErrorList{
				field.Invalid(field.NewPath("status", "devices").Index(0).Child("data").Index(0), "<value omitted>", "error parsing data: invalid character 'o' in literal false (expecting 'a')"),
			},
			oldClaim: func() *resource.ResourceClaim { return validAllocatedClaim }(),
			update: func(claim *resource.ResourceClaim) *resource.ResourceClaim {
				claim = claim.DeepCopy()
