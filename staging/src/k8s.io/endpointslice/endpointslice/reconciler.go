...
func (r *Reconciler) Reconcile(
	...
	return r.finalize(ownerGVK, ownerObjectMeta, slicesToCreate, slicesToUpdate, slicesToDelete, trafficDistribution, triggerTime)
}
...
func (r *Reconciler) reconcileByAddressType(
	...
	for _, desired := range desiredEndpointSlices {
		...
		spMetrics.Set(epHash, metrics.EfficiencyInfo{
	...
	if r.placeholderEnabled && len(existingSlices) == len(slicesToDelete) && len(slicesToCreate) < 1 {
		...
		placeholderSlice := newEndpointSlice(logger, r.controllerName, ownerGVK, ownerObjectMeta, []discovery.EndpointPort{}, addressType, setEndpointSliceLabelsAnnotationsFunc)
	...
	r.metricsCache.UpdateServicePortCache(ownerNN, spMetrics)
	...
	for _, event := range events {
		r.eventRecorder.Event(nil, event.EventType, event.Reason, event.Message) // TODO
	}
...
func (r *Reconciler) finalize(
	...
	if ownerObjectMeta.GetDeletionTimestamp() == nil {
		for _, endpointSlice := range slicesToCreate {
			...
			if err != nil {
				...
				return fmt.Errorf("failed to create EndpointSlice for %s %s/%s: %v", ownerGVK.Kind, ownerObjectMeta.GetNamespace(), ownerObjectMeta.GetName(), err)
			}
	...
	for _, endpointSlice := range slicesToUpdate {
		...
		if err != nil {
			return fmt.Errorf("failed to update %s EndpointSlice for %s %s/%s: %v", ownerGVK.Kind, endpointSlice.Name, ownerObjectMeta.GetNamespace(), ownerObjectMeta.GetName(), err)
		}
	...
	for _, endpointSlice := range slicesToDelete {
		...
		if err != nil {
			return fmt.Errorf("failed to delete %s EndpointSlice for %s %s/%s: %v", ownerGVK.Kind, endpointSlice.Name, ownerObjectMeta.GetNamespace(), ownerObjectMeta.GetName(), err)
		}
...
func (r *Reconciler) reconcileByPortMapping(
	...
	for desiredSet.Len() > 0 {
		...
		if sliceToFill == nil {
			sliceToFill = newEndpointSlice(logger, r.controllerName, ownerGVK, ownerObjectMeta, endpointPorts, addressType, setEndpointSliceLabelsAnnotationsFunc)
		} else {
